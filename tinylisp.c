...
#define debug(x) printf("->");print(x);puts("");
...
L closure(L v,L x,L e) { 
	return box(CLOS,ord(pair(cadr(v),x,equ(e,env)?nil:e))); }
...
L f_cons(L t,L e) { return t=evlis(t,e),cons(car(t),cadr(t)); }
...
L f_lt(L t,L e) { return t=evlis(t,e),car(t) - cadr(t) < 0?tru:nil; }
L f_eq(L t,L e) { return t=evlis(t,e),equ(car(t),cadr(t))?tru:nil; }
...
// L f_cond(L t,L e) { while (not(eval(car(car(t)),e))) t=cdr(t); return eval(cadr(car(t)),e); }
L f_if(L t,L e) { return eval(cadr(not(eval(car(t),e))?cdr(t):t),e); }
L f_leta(L t,L e) { for (;let(t); t=cdr(t)) e=pair(car(car(t)),eval(cadr(car(t)),e),e); return eval(car(t),e); }
L f_ARGS(L t,L e) { for (L p;T(t)!=NIL;t=cdr(t)) p=car(cdr(car(t))), e=pair(car(p),eval(cadr(p),e),e); return eval(cadr(t),e); }
L f_FUNCTION(L t,L e) { return closure(f_quote(t,e),cadr(t),e); }
L f_GLOBAL(L t,L e) { env=pair(car(t),eval(cadr(t),e),env); return car(t); }
...
L f_SETG(L t,L e) { env=pair(eval(car(t),e),eval(cadr(t),e),env); return car(t); }
L f_SET(L t,L e) { 
	L v=eval(car(t),e),val=eval(cadr(t), e);
...
L f_DEFINE(L t,L e) {
	L name=car(t), params=cadr(t), body=cddr(t), x, s=nil;
	for (L b=body; !not(b); s=cons(car(b),s),b=cdr(b));
	x=car(s), s=cdr(s);	
	for (;!not(s); x=cons(box(GLOBAL,ord(atom("if"))),cons(car(s),cons(x,cons(x,nil)))),s=cdr(s));	
	env=pair(name,closure(cons(atom("quote"),cons(params,nil)),x,e),env);
	return name;
}
L f_MAPF(L t,L e) {
	L apply(L f,L t,L e);
...
L zillist() { L x; return scan()=='>'?nil:(x=parse(),cons(x,zillist())); }
...
L bind(L v,L t,L e) {
	if (not(v)) return e;
	if (T(v)!=CONS) return pair(v,t,e); // Simple atom binding
	L args=t, tuple_var=nil;
	enum {REQUIRED, TUPLE, OPTIONAL, AUX} mode=REQUIRED;
	// Process parameter list
	for (L p=cadr(v); T(p)==CONS; p=cdr(p)) {
		L param=car(p),var,val;
		// Check for mode markers
		if (T(param)==STRING) {
			char *s=A+ord(param);
			if (!strcmp(s,"TUPLE")) {mode=TUPLE;continue;}
			else if (!strcmp(s,"OPTIONAL")) {mode=OPTIONAL;continue;}
			else if (!strcmp(s,"AUX")) {mode=AUX;continue;}
		}
		switch(mode) {
			case REQUIRED:
				if (not(args)) return err; // Not enough arguments
				e=pair(param,car(args),e);
				args=cdr(args);
				break;
				
			case TUPLE:
				tuple_var=param;
				e=pair(tuple_var,args,e);
				args=nil;
				mode=OPTIONAL; // After TUPLE, continue to OPTIONAL
				break;
			...
			case OPTIONAL:
				if (T(param)==CONS) {
					...
					param = cadr(param);
					var=car(param);
					val=not(args)?eval(cadr(param),e):car(args);
				} else {
					// var alone defaults to nil
					var=param;
					val=not(args)?nil:car(args);
				}
				e=pair(var,val,e);
				if (!not(args)) args=cdr(args);
				break;
			...
			case AUX:
				if (T(param)==CONS) {
					// (var initial-value)
					param = cadr(param);
					var=car(param),val=eval(cadr(param),e);
				} else {
					// var alone defaults to nil
					var=param,val=nil;
				}
